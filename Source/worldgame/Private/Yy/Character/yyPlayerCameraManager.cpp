...
/* 初始化并隐藏 CameraBehavior */
AyyPlayerCameraManager::AyyPlayerCameraManager()
{
 ...
 * 初始化CameraAnim状态标志
 */
void AyyPlayerCameraManager::OnPossess(AyyBaseCharacter* NewCharacter)
{
	check(NewCharacter);
	ControlledCharacter = NewCharacter;
	UyyPlayerCameraBehavior* CastedBehv = Cast<UyyPlayerCameraBehavior>(CameraBehavior->GetAnimInstance());
	if (CastedBehv)
	{
...
float AyyPlayerCameraManager::GetCameraBehaviorParam(FName CurveName) const
{
...
/* 帧更新函数 */
bool AyyPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
...
void AyyPlayerCameraManager::UpdateViewTargetInternal(FTViewTarget& OutVT, float DeltaTime)
{
	...
	if (OutVT.Target)
	{
		...
		float OutFOV;
		if (OutVT.Target->IsA<AyyBaseCharacter>())
		{
...
FVector AyyPlayerCameraManager::CalculateAxisIndependentLag(FVector CurrentLocation, FVector TargetLocation,
